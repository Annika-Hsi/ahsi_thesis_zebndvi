...
ndvi_match <- function(start_date, df, tiff_path_a, tiff_path_b) {
  ...
  for (row in 1:nrow(df)) {
    ...
    # to get NDVI index, use formula:
    # tiff width * vertical index + horizontal index
    tiff_width <- dim(NDVI)[2]
    vi <- round((df_lat - origin_lat)/avg_l_lat)
    hi <- round((df_long - origin_long)/avg_l_long)
    tiff_ndvis <- c(dplyr::select(NDVI_df, all_of("ndvi")))
    df_NDVIs[row] <- tiff_ndvis[[1]][tiff_width*vi + hi]
  }
  df_NDVIs
}
