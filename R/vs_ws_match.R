...
vs_ws_match <- function(ws, vs) {
  ...
  for (i in 1:nrow(vs)) {
    ...
    # get dates for vegetation transect and zebra white sheet data
    ws$Date <- as.Date(ws$Date, "%d/%m/%Y")
    vs$Date <- as.Date(vs$Date, "%d/%m/%Y")
    Date <- vs$Date[i]

    # get rows in white sheets that have same date as current transect
    curr_ws <- dplyr::filter(ws, Date == vs$Date[i])

    # find zebra sighting on that day with the closest distance to the transect
    min_dist <- Inf
